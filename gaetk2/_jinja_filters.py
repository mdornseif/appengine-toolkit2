...
Copyright (c) 2010, 2012, 2014, 2017 HUDORA. MIT Licensed.
"""
import decimal
import json
import logging
...
def filter_g2kg(value):
    """Wandelt meist g in kg um, aber auch in andere Einheiten."""
    if not value:
        return value
    elif value < 100:
        return Markup("{:d}&thinsp;g".format(value))
    elif value < 1000 * 50:
        return Markup("{:.2f}&thinsp;kg".format(value / 1000.0))
    elif value < 1000 * 1000:
        return Markup("{:.1f}&thinsp;kg".format(value / 1000.0))
    else:
        return Markup("{:.1f}&thinsp;t".format(value / 1000.0 ** 2))
...
def datastore_query_link(entity, attr):
    """HTML Link zur Google Datastore Query erzeugen."""
...
def _datastore_query(entity, attr):
    """URL zur Google Datastore Query erzeugen."""
    value = getattr(entity, attr, '')
    query = {
...
def register_custom_filters(jinjaenv):
    ...
    jinjaenv.filters['g2kg'] = filter_g2kg
